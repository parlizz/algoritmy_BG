...
for id, func_name in enumerate(name_list):
    ...
    if id == 0 or id == 1:
        ...
        print(
            f"{func_name} -\t{timeit.timeit(stmt=func_name + f'(dict_for_test_one, dict_for_test_two)', setup=f'from __main__ import {func_name}', number=num_time, globals=globals())}")
    elif id == 11:
        print(
            f"{func_name} -\t{timeit.timeit(stmt=func_name + f'(o_dict_for_test_one, o_dict_for_test_two)', setup=f'from __main__ import {func_name}', number=num_time, globals=globals())}")
    else:
        if id % 2 == 0:
            print(
                f"{func_name} -\t{timeit.timeit(stmt=func_name + f'(dict_for_test_one)', setup=f'from __main__ import {func_name}', number=num_time, globals=globals())}")
        else:
            print(
                f"{func_name} -\t{timeit.timeit(stmt=func_name + f'(o_dict_for_test_one)', setup=f'from __main__ import {func_name}', number=num_time, globals=globals())}")
